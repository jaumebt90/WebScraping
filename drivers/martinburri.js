...
async function getFirstLinks() {
  let url1 = "https://martinburrirealestate.com/es/immobilien/page/";
  let i;
  let url2 =
    "/?availabilities%5B0%5D=active&availabilities%5B1%5D=reserved&availabilities%5B2%5D=taken&categories%5B0%5D&locations&salestypes";
  let arrayResult = [];
  for (i = 1; i <= 5; i++) {
    ...
    const enlaces = await page.evaluate(() => {
      const elements = document.querySelectorAll(".casawp-property a");
      const links = [];
...
async function getFirstDetalles(input) {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    const enlaces = await page.evaluate(() => {
      const elements = document.querySelectorAll(".casawp-single");
      const links = [];
      for (let element of elements) {
        links.push(element.href);
      }
