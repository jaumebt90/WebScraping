...
async function getFirstLinks() {
  let url1 = "https://www.lavin-estates.com/property-search/results/p";
  let i;
  let url2 = "?island=Mallorca";
  let arrayResult = [];
  for (i = 1; i <= 60; i++) {
    ...
    const enlaces = await page.evaluate(() => {
      const elements = document.querySelectorAll(".results-bar a");
      const links = [];
...
async function getFirstDetalles(input) {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    const enlaces = await page.evaluate(() => {
      const elements = document.querySelectorAll("#property-details");
      const links = [];
      for (let element of elements) {
        links.push(element.href);
      }
